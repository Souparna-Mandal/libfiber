...
void sched_lock_acquire(struct sched_lock *lock)
{
    ...
    if (lock->slice_acquired == 0){        
        ...
        gettimeofday(&lock->lock_stat->start_ticks, NULL);
        set_lock_fiber_data((void *)lock, lock->lock_stat->banned_until,
        lock->lock_stat->slice_size, 
        lock->lock_stat->start_ticks,
        lock->lock_stat->end_ticks, 
        lock->holder);

        // Compute the slice end time.
        timeval_add(&lock->slice_end_time,
             &lock->lock_stat->start_ticks,
            &lock->lock_stat->slice_size);
...
void sched_lock_release(struct sched_lock *lock)
{
    ...
    gettimeofday(&lock->lock_stat->end_ticks, NULL); // we use the last possible end-ticks 
    // performance issue that we set everytime, but should be okay I THINK, need to verify 
    if (timercmp(&lock->lock_stat->end_ticks, &lock->slice_end_time,>)){ // enter if slice has expired
        set_lock_fiber_data((void *)lock,
        lock->lock_stat->banned_until,
        lock->lock_stat->slice_size, 
        lock->lock_stat->start_ticks,
        lock->lock_stat->end_ticks, 
        lock->holder);
...
void ban_fibers(struct sched_lock *lock , fiber_t* fiber){
    ...
    struct lock_stats lock_banning;
    if (get_lock_fiber_data((void*)lock, &lock_banning, NULL) == 0){
        printf("Error: No Stats, something is wrong.\n");
        abort();
    }
    if (lock->holder != fiber){
      return;
    } else if ((nthreads > 1) && (lock_banning.start_ticks.tv_sec > 0) &&
               (lock_banning.end_ticks.tv_sec > 0)) {  // don't ban in the start
      /* Expand ban tvime by (cs_length * num_threads). */
      cs_length = time_difference(&lock_banning.start_ticks, &lock_banning.end_ticks);
    ...
    //     fiber, cs_length, lock->start_ticks.tv_sec, lock->start_ticks.tv_usec, lock->end_ticks.tv_sec, lock->end_ticks.tv_usec);
      ...
      timeval_add(&banned_until, &lock_banning.end_ticks, &time_adder);
      set_lock_fiber_data((void *)lock, banned_until,
                          (struct timeval){0, SLICE_SIZE_US},(struct timeval){0, 0},(struct timeval){0, 0},  fiber);
    } else {
      /* If only one fiber, no ban needed. */
      set_lock_fiber_data((void *)lock, lock_banning.end_ticks,
                          (struct timeval){0, SLICE_SIZE_US},(struct timeval){0, 0},(struct timeval){0, 0}, fiber);
    }
}